...
{
	$pathtosend='/images/'.$file->getClientOriginalName();
    ...
    } else
    {
        ...
        // read image from temporary file
         $path_image = public_path().'/images/covers/'.$file->getClientOriginalName();
          if (File::exists($path_image))
{
	$pathtosend='/images/covers/'.$file->getClientOriginalName();
...
if ($validator->fails()):
    ...
    return view('cms/add-guser')->with('error',"some thing is missing or wrong");
elseif($request->password != $request->cpassword):
	return view('cms/add-guser')->with('error',"Password mismatch error...");
else:
...
$adminusers=Users::where('gender','=',$request->q)->get();
...
$adminusers_count=Users::all()->count();
endif;
 return view('cms/general-users-list')->with('users',$adminusers)->with('selected_items',$adminusers_count);
...
$book->url=preg_replace('/[^A-Za-z0-9-]+/', '-', $book->url);
