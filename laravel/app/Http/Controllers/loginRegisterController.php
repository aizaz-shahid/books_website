...
/*if (Auth::attempt(['email' => $request->email, 'password' => $data])) {
	...
	->with('Error3', $error)->with('resultSet11',$result11)->with('resultSet12',$result12);
}
...
if(!$exists){
	...
	//matching the password and re-enter password fields value
		...
		Mail::send('register-email',['name' => $request->firstname],function($m){
			$m->from('from@example.com','duz');
			$m->to('aizazshahid47@gmail.com','Shahid Iqbal')->subject('Congragulations!');
		});

		Mail::send('register-email',['name' => 'Admin','Name' => $request->firstname],function($m){
			$m->from('from@example.com','Aizaz');
			$m->to('admin@obooko.com','Shahid Iqbal')->subject('Congragulations!');
		});
