...
$blog->save();
redirect('/list_blog')->with('status',"Added Successfully");
		else:
return redirect('/list_blog')->with('status',"Failed");
...
return view('cms/edit-book')->with('book',$book);
...
if($request->bkid == "" || $request->bkid == " ")
 ...
 $book_synopsis=$request->bkdesc;
	if($request->hasFile('pdf') && $book->url == "")
        { 
            $request->file('pdf')->move(public_path().'/books/pdf', $book->url.'.pdf');
		       $book->pdf=$book->url;
         }
if($request->hasFile('kindle') && $book->url == "")
        { 
            $request->file('kindle')->move(public_path().'/books/kindle', $book->url.'.kindle');
		       $book->kindle=$book->url;
         }
         if($request->hasFile('epub') && $book->url == "")
        { 
            $request->file('epub')->move(public_path().'/books/epub', $book->url.'.epub');
		       $book->epub=$book->url;
         }
...
$book->cover_image=$request->cimage;
$book->synopsis=$request->cshortdesc;
$book->url=$request->slug;
$cat="";
$i=0;
