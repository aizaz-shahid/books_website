...
->with('resultSet12', $result12)->with('Categories',$categories);
	...
	if ($result->type == "author") {
		...
		Mail::send('book-submit-email',['book' => $request->title,'category' => $categ->name, 'author' => $mail_name->author_name],	function($m) use ($result){
				$m->from('admin@obooko.com','admin');
				$m->to($result->email,$result->first_name)->subject('Author New Book!');
			});	
	}
	elseif ($result->type == "free") {
	...
	->update(['type' => 'author']);
		...
		Mail::send('book-submit-email',['book' => $request->title,'category' => $categ->name, 'author' => $mail_name->author_name],	function($m) use ($result){
				$m->from('admin@obooko.com','admin');
				$m->to($result->email,$result->first_name)->subject('Author First Book!');
			});
	}
...
if ($validator->fails())
{
	...
	return redirect('author-updates')->with('Error',$error);
}
else {
if ($title) {
    ...
    if($request->hasFile('kindle'))
		...
		File::delete($filename);
              ...
              $request->file('kindle')->move(public_path().'/books/kindle', $slug.'-obooko.'.$request->file('pdf')->getClientOriginalExtension());
				DB::table('books')
				->where('book_id', $title->book_id)
				->update(['kindle' => $slug]);
         }
    ...
    if($request->hasFile('epub'))
		...
		File::delete($filename);
              $request->file('epub')->move(public_path().'/books/epub', $slug.'-obooko.'.$request->file('pdf')->getClientOriginalExtension());
				DB::table('books')
				->where('book_id', $title->book_id)
				->update(['epub' => $slug]);
         }
	...
	if($request->hasFile('word'))
        { 
              ...
              $request->file('word')->move(public_path().'/books/word', $slug.'-obooko.'.$request->file('pdf')->getClientOriginalExtension());
				DB::table('books')
				->where('book_id', $title->book_id)
				->update(['word' => $slug]);
         }
...
Mail::send('book-update-email',['book' => $request->title, 'author' => $title->author_name],	function($m) use ($result){
				$m->from('admin@obooko.com','admin');
				$m->to($result->email,$result->first_name)->subject('Book Update!');
			});
...
if ($validator->fails()) {
	...
	return redirect('send-feedback')->with('Error', $error);            
		...
		if ($request->email == $request->c_email) {	
			Mail::send('feedback-email',['name' => $book->title,'subject' => $request->name, 'email' => $request->email],	function($m) use ($author){
				$m->from('admin@obooko.com','admin');
